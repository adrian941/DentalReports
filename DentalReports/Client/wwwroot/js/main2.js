...
async function init() {
	...
	async function fetchData() {
		...
		try {
			...
			if (response.ok) {
				...
				window.objects = jsonResponse.objects;
				window.cameraPosition = response.json().cameraPosition;
				window.cameraRotation = response.json().cameraRotation;
			} else {
	...
	function findMaterialFromFileName(fileName) {
		for (var i = 0; i < window.objects.length; i++) {
			if (fileName.endsWith(window.objects[i].FileName)) {
